...
`menu.open()` displays the menu without a gesture and lets the keyboard operate it, and `menu.close()` closes it. Pass `{ position }`, in viewport pixels, to center it somewhere other than the parent's center. Events from such a menu have `mode: 'standalone'` and a `position` of `undefined`, so check `mode` before reading `position`. When a stroke was recognized, `open`, `select`, and `cancel` events have a `recognition` with the stroke and how it was cut into corners and segments. `cancel` events have a `reason`: `no-selection`, `interrupted` when the browser cancels the pointer, or `dismissed` when a menu shown with `open()` is closed. The package exports the new `MarkingMenuCancelReason`, `MarkingMenuOpenOptions`, `MarkingMenuRecognition`, `MarkingMenuStrokeAnalysis`, and `MarkingMenuStrokeSegment` types.

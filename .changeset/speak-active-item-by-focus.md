...
A menu shown with `menu.open()` takes focus the same way. Arrow keys, Home, End, Enter, and Escape move through it, and Tab closes it. Pass `{ focus: false }` to display it without moving focus.

...
`select` carries the selected item as `event.selection`, including its `id` and `label`. `move`, `change`, and `cancel` carry the item under the pointer as `event.activeItem`, and `change` also carries `event.previousActiveItem`. Both are `undefined` when no item is under the pointer. `open` also carries `event.willAutoFocus`: `false` for a menu opened with [`autoFocus: false`](#opening-without-autofocus), `true` otherwise, including for every submenu. `cancel` carries `event.reason`: `no-selection` when a gesture ends with nothing to select, `interrupted` when the browser cancels the pointer, or `dismissed` when `close()`, Escape, Tab, or focus leaving the menu closes one shown with `open()`.

Every event includes `mode`: `startup` while waiting for movement or a pause, `novice` while using a visible menu, `expert` while drawing a gesture, or `standalone` for a menu shown with [`open()`](#open-and-close), operated with the keyboard or a pointer. Every event also includes `source`, what caused it: `gesture` for a drawn stroke, `pointer`, `keyboard`, `api` for an `open()` or `close()` call or the focus `open()` gives the first item, or `focus-loss`. Every event also includes `position`, a viewport `[x, y]` pair; outside `standalone` mode it is always one, and inside it, only when `source` is `pointer`. Checking `mode` narrows `position` in TypeScript:
...
| `autoFocus` | `true`               | Whether the menu takes focus as it opens. See [below](#opening-without-autofocus).   |
...
Run `yarn test` for the unit and browser suites. `yarn test:coverage` runs the coverage suite on Chromium, `yarn test:cross-browser` runs the Firefox and WebKit interaction tests, and `yarn test:dist` builds the demo and runs tests against the built package and deployed demo. If CI has already built the artifacts, use `yarn test:dist:only` to skip rebuilding. Install the browsers with `yarn playwright install chromium firefox webkit`.

Visual baselines are committed for the pinned container only (`-chromium-linux.png`). Outside it, Vitest writes a fresh platform-local baseline (e.g. `-chromium-darwin.png`, gitignored) on first run. Generate that baseline before making a rendering change, so a later run can compare against it. Inside the container, the checked-in `-chromium-linux.png` files are the references.

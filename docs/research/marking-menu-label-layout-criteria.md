...
The best first implementation to test is an **adaptive candidate-position solver backed by exact constraint search**. Generate meaningful plate placements for each fixed item direction; precompute whether each placement is legal and which pairs conflict; then select the complete layout jointly with a constraint-programming or branch-and-bound solver. Per-label candidate sets are only search domains: plates are never placed or optimized independently. Solve the objectives sequentially, not as one weighted sum. This guarantees hard constraints and global optimality *within the generated candidate set* when the search completes, can be deterministic, and is a much smaller engineering commitment than a continuous global mixed-integer nonlinear model. The planned breadth increase from eight to at least twelve items makes scalability an explicit acceptance test: the prototype must establish the candidate count and search budget at 12, 16, and a plausible upper stress bound rather than assuming exact search will always finish. A continuous constrained pass may polish the selected layout, but only if it remains inside the same feasible region and is followed by exact geometric validation.
...
### 1. Candidate positions plus exact constraint search — recommended first
...
### 2. Exact finite solve followed by constrained continuous polish — recommended second
...
### 3. Global mixed-integer nonlinear optimization — useful as an offline oracle
...
### 4. Constrained local nonlinear optimization — useful only inside a known feasible cell
...
### 5. Force/energy relaxation — valuable baseline or candidate generator, not final authority
...
### 6. Simulated annealing — strong heuristic baseline, unnecessary as the sole solver here
...
### 7. Boundary, contour, and orbital dynamic programs — instructive, but their geometry is not ours

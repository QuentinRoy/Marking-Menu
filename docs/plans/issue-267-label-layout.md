...
## Select the production algorithm

Use this decision tree:

- Ship candidate search with adaptive refinement when it is valid, deterministic, fast enough at the intended maximum, and visually preferred.
- Add continuous polish only when it fixes a visible candidate-grid problem and remains valid and deterministic.
- Use a deterministic search-work budget and a validated incumbent when proving the finite optimum is too expensive.
...
- determinism tests;
- optimality checks on small cases that can be enumerated;
